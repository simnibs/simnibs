...
% John Ashburner
% $Id: cat_spm_smoothto8bit.m 1138 2017-06-12 14:26:32Z dahnke $
...
img       = single(spm_read_vols(V)); img(isnan(img(:)) | isinf(img(:))) = 0; 

% use the more exact spm_smooth routine
spm_smooth(img,img,fwhm./vx);

mx        = max(img(:));
mn        = min(img(:));
VO.pinfo  = repmat([1;0],1,size(img,3));
VO.dat    = uint8(round((img-mn)*(255/(mx-mn))));
