...
void vol_localstat(float *M, float *M2, float*M3, float *D, unsigned char *B,  int *sL, int nh, int st, int verb)
{
  ...
  for (i=0;i<nL;i++) { 
		if (D[i]==-FLT_MAX || isnan(D[i])) D[i]=0.0; 	/* correction of non-visited or other incorrect voxels */
    M[i]  = 0.0;
  } 
  ...
  for (z=0;z<sL[2];z++) for (y=0;y<sL[1];y++) for (x=0;x<sL[0];x++) {
    ind = index(x,y,z,sL);
    if ( B[ind] && isnan(D[ind])==0 && isinf(D[ind])==0  ) {
      n  = 0;
      ...
      for (i=-nh;i<=nh;i++) for (j=-nh;j<=nh;j++) for (k=-nh;k<=nh;k++) {
        ...
        if ( ((x+i)>=0) && ((x+i)<sL[0]) && ((y+j)>=0) && ((y+j)<sL[1]) && ((z+k)>=0) && ((z+k)<sL[2])) {
          ...
          if ( ( B[ni] || st==7 ) && isnan(D[ni])==0 && isinf(D[ni])==0 ) {
     ...
     if (st==8) {
      ...
      if (st==9) {
        ...
        for (di=0;di<=2;di++) { 
          for (i=-nh*2;i<=nh*2;i++) {
            ...
            if ( (x+i)>=0 && (x+i)<sL[0] ) {
              if (B[ni] && isnan(D[ni])==0 && isinf(D[ni])==0 ) {
      ...
      if ((M[ind]==-FLT_MAX) || (D[ind]==FLT_MAX) || (isnan(D[ind])) ) M[ind]=0.0;
  ...
  for (i=0;i<nL;i++) { 
		if ( M[i]==-FLT_MAX || M[i]==FLT_MAX || isnan(M[i]) ) M[i]=0.0; 	/* correction of non-visited or other incorrect voxels */
	} 
