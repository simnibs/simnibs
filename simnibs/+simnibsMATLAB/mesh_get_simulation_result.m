...
[~,~,ext] = fileparts(fname);
if strcmpi(ext,'.msh')
    m=simnibsMATLAB.mesh_load_gmsh4(fullfile(pname,fname));
else
    m=simnibsMATLAB.mesh_load_fsresults(fullfile(pname,fname));
end


% plot histogram of field in GM (based on volume for .msh; area for surface data)
simnibsMATLAB.mesh_get_histogram(m)

% show field on GM surface
simnibsMATLAB.mesh_show_surface(m)

% extract a few key values
simnibsMATLAB.mesh_get_fieldpeaks_and_focality(m);
