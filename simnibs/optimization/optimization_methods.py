...
class TESOptimizationProblem(TESConstraints):
    ...
    def __init__(self, leadfield, max_total_current=1e4, max_el_current=1e4, weights=None):
        ...
        assert self.weights.shape[0] == self.n_target, "Please define one weight per leadfield element"
    ...
    def _calc_l_vec(self, target_indices, target_direction, weights):
        ...
        assert len(target_indices) == len(target_direction), "Please define one direction per target"

        target_direction = target_direction/np.linalg.norm(target_direction, axis=1, keepdims=True)
    ...
    def _calc_Q_mat(
            self,
            indices = None, #: None | npt.ArrayLike = None,
            weights = None, #: None | npt.ArrayLike = None
        ):
...
class TESLinearElecConstrained(TESLinearConstrained):
    ...
    def solve(self, log_level=20, eps_bb=1e-1, max_bb_iter=100, init_strategy='compact'):
...
class TESDistributedElecConstrained(TESDistributed):
    ...
    def solve(self, log_level=20, eps_bb=1e-1, max_bb_iter=500, init_strategy='compact'):
        # Heuristically eliminate electrodes
        max_el_current = min(self.max_el_current, self.max_total_current)
        el = np.arange(self.n)
        if init_strategy == 'compact':
...
def _linear_constrained_tes_opt(
        ...
        assert l.shape[0] == target_mean.shape[0], "Please specify one target mean per target"
        assert l.shape[1] == Q.shape[0]

        scale = max_el_current
        ...
        # Do the QP
        eps = 1e-3 * np.abs(x_).max() * scale # x_ is scaled by 1/scale
        Q_ = np.block([[Q, -Q], [-Q, Q]])
...
def _norm_opt_x0(
    ...
    Q : _type_
        _description_
    max_el_current : _type_
        _description_
    max_total_current : _type_
        _description_
    extra_ineq : _type_, optional
        _description_, by default None
    extra_eq : _type_, optional
        _description_, by default None
...
def _active_set_QP(l, Q, C, d, x0, eps=1e-5, A=None, b=None):
    ...
    if A is not None and b is not None:
        assert np.allclose(A.dot(x0), b, atol=eps), "Infeasible start!"
