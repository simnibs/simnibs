...
class TMSoptimize:
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True, return_n_max=1):
        ...
        if os.path.isdir(dir_name):
            ...
            if len(g) > 0 and not allow_multiple_runs:
                raise IOError(
                    ...
                    " the simnibs_simulation*.mat files from the folder : {0}".format(
                        dir_name
                    )
                )
        ...
        if len(target_region) == 0:
            raise ValueError(
                "Did not find any elements within the defined target region"
            )
        ...
        m.write(fn_target)
        v = m.view(visible_tags=self.tissues, visible_fields="all")
    ...
    def _ADM_optimize(self, cond_field, target_region):
        ...
        def calc_dipole_J(dipole_dir):
            ...
            b = S.assemble_rhs(
                dip_pos,
                np.atleast_2d(dipole_dir)
                * 1e-9
                * np.atleast_1d(vols[target_region])[:, None],
                "partial integration",
            )
            if b.ndim == 2:
                b = b.sum(1)
...
def _create_grid(
    mesh, pos, distance, radius, resolution_pos, scalp_normals_smoothing_steps=20
):
    ...
    elm_center_zeromean = elm_center[elm_mask_roi] - np.mean(
        elm_center[elm_mask_roi], axis=0
    )
...
def _rotate_system(R, angle_limits, angle_res):
    ...
    angles = np.deg2rad(angles[(angles > -180.1) * (angles < 180.0)])
    matrices = []
    for a in angles:
        Rz = np.array(
            (
...
def get_opt_grid(
    ...
    if handle_direction_ref is None:
        y_seed = np.array([0.0, 1.0, 0.0])
    else:
        y_seed = np.array(handle_direction_ref) - np.array(pos)
        if np.isclose(np.linalg.norm(y_seed), 0.0):
            raise ValueError(
                "The coil Y axis reference is too close to the coil center! "
            )
    ...
    for p, n in zip(coords_mapped, coords_normals):
        ...
        for r in rotated:
            matrices.append(np.vstack((np.hstack((r, p[:, None])), [0, 0, 0, 1])))
...
def get_opt_grid_ADM(
    ...
    directions = np.array(_rotate_system(np.eye(3), angle_limits, resolution_angle))[
        :, 1
    ].T
