...
def resolve_subject_id_path(subid):
    ...
    return m2m_dir if m2m_dir.name.startswith('m2m_') else m2m_dir.with_name(f"m2m_{m2m_dir.name}")


subid = CommandLineArgument(['subid'], dict(
    type = str,
...
subsampling = CommandLineArgument(["-s", "--subsampling"], dict(
    type = int,
    default = 10000,
...
fsaverage = CommandLineArgument(["--fsaverage"], dict(
    type = int,
    default = 5,
    choices = [5, 6, 7],
    help = """
...
leadfield = CommandLineArgument(['leadfield'], dict(
        type = str,
        help = "Name of the leadfield to process (HDF5 file)."
    ))

#### MNE, FieldTrip

info_mne = CommandLineArgument(["info"], dict(
    type = str,
    help = """
...
trans_mne = CommandLineArgument(["trans"], dict(
    type = str,
    help = """
...
info_fieldtrip = CommandLineArgument(["info"], dict(
    type = str,
    help = """
...
trans_fieldtrip = CommandLineArgument(["trans"], dict(
    type = str,
    help = """
