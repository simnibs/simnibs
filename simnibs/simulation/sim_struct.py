...
class TMSOptimize():
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True):
        ...
        pos_matrices = optim_tms.get_opt_grid(
            self.mesh, self.pos,
        ...
        target_region = optim_tms.define_target_region(
            self.mesh,
            self.target,
            self.target_size,
            self.tissues
        ...
        # write mesh with target and ge file with grid
        m = self.mesh
        ...
        m.elmdata = []
        # Write out the grid
        optim_tms.plot_matsimnibs_list(
            pos_matrices,
