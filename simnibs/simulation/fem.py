...
# TODO: properly implement the following methods to compute e-fields from potential very fast
...
local_node_coords = node_coordinates[:, mesh.node_numbers - 1]
...
class Surface:
    # TODO: replace filename with points
    def __init__(self, filename, msh, gradient=None, out_fill=0):
        pts = read_gifti_surface(filename).nodes.node_coord

        assert pts.shape[1] == 3

        self.siz = pts.shape[0]

        self.filename = filename

        self._get_sF_matrix(msh, pts, out_fill)

        if not gradient:
            # get the lengths of the tetrahedral edges

            local_dist, _ = _get_edge_lengths(msh)
        ...
        self.node_index_list = msh.elm.node_number_list[self.idx] - 1
...
​
        ...
        Args:
            ...
            out_fill (number or None): Value to be goven to points outside the volume. If None then use nearest neighbor assigns the nearest value; otherwise assign to out_fill, for example 0)
