SimNIBS includes 3rd party open-source and free code and binaries from several projects and research groups:


ADMlib
=======
github.com/luisgo/Auxiliary_dipole_method
**License and link to online license text (if existing):**
GNU General Public Licence (GPL-v2.0), for non-commercial and academic purposes only
Commercial Licence for commercial use without the GPL-v2.0 restrictions.
**Copyright** 
(c) copyrights Duke University. 
As a recipient of this software, you may choose which Licence to receive the code under.
Outside contributions to the Duke owned code base cannot be accepted unless the contributor transfers the copyright to those changes over to Duke University.
To enter a commercial Licence agreement, please contact the Digital Innovations department at Duke Office of Licensing and Ventures (https://olv.duke.edu/software/) at olvquestions@duke.edu with reference to “OLV File No. 7148” in your email.
Please note that this software is distributed AS IS, WITHOUT ANY WARRANTY; and without the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.


BLAS
=======
www.netlib.org/blas
**License and link to online license text (if existing):**
custom license, “freely-available software package”
www.netlib.org/blas/#_licensing
**Copyright** 
?

   
CAT12        
=======
www.neuro.uni-jena.de/cat
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 2 or later
**Copyright** 
Christian Gaser and Robert Dahnke (Jena University Hospital, Germany)        


CGAL
=======
www.cgal.org
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 3 or later
Or a commercial license
www.cgal.org/license.html
**Copyright** 
Copyright (c) 1995-2024 The CGAL Project


charm-gems
===========
github.com/simnibs/charm-gems
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 3 or later
github.com/simnibs/charm-gems?tab=GPL-3.0-1-ov-file#readme
**Copyright** 
Oula Puonti and Koen Van Leemput


Cygwin API library
===============
www.cygwin.com
**License and link to online license text (if existing):**
GNU LGPLv3
www.cygwin.com/licensing.html
**Copyright** 
Cygwin project
**Remarks** 
used for meshfix on Windows


FMM3D
=======
github.com/flatironinstitute/FMM3D
**License and link to online license text (if existing):**
Apache License, Version 2.0
github.com/flatironinstitute/FMM3D?tab=License-1-ov-file#readme
**Copyright** 
Copyright (C) 2019-2023 The Simons Foundation, Inc., FMM3D Development Team        


FSL
=======
fsl.fmrib.ox.ac.uk/fsl/docs/#/
**License and link to online license text (if existing):**
custom license, “free for non-commercial purposes”
fsl.fmrib.ox.ac.uk/fsl/docs/#/license
**Copyright** 
(c) 2018, The University of Oxford
**Remarks** 
used by dwi2cond


GMP
=======
www.gmplib.org
**License and link to online license text (if existing):**
Dual-licensed under GNU LGPL v3 and GNU GPL v2
www.gmplib.org
**Copyright** 
gmplib.org/manual/Contributors


gmsh
=======
www.gmsh.info
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 2 or later
www.gmsh.info/LICENSE.txt
**Copyright** 
Christophe Geuzaine <cgeuzaine at uliege.be> and Jean-Francois Remacle  <jean-francois.remacle at uclouvain.be>
**Remarks** 
used for results visualisation


Intel MKL
=======
www.intel.com/content/www/us/en/developer/tools/oneapi/onemkl-download.html
**License and link to online license text (if existing):**
Intel Simplified Software License
www.intel.com/content/www/us/en/developer/articles/license/end-user-license-agreement.html#inpage-nav-2
**Copyright** 
Intel
**Remarks** 
Intel MKL Pardiso solver is used for FEM on Windows and Linux


Matplotlib
=======
matplotlib.org
**License and link to online license text (if existing):**
PSF license
matplotlib.org/stable/project/license.html
**Copyright** 
Matplotlib Development Team


meshfix
=======
pymeshfix.pyvista.org
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 3 or later
Or a commercial license
pymeshfix.pyvista.org/#copyright
**Copyright** 
Copyright(C) 2010: IMATI-GE / CNR        


Mmg
=======
www.mmgtools.org
**License and link to online license text (if existing):**
GNU Lesser General Public License (LGPL)
raw.githubusercontent.com/MmgTools/mmg/master/LICENSE
**Copyright** 
Mmg consortium


MPFR
=======
www.mpfr.org
**License and link to online license text (if existing):**
GNU Lesser GPL, version 3 or later
www.mpfr.org
**Copyright** 
registered in France by the Agence de Protection des Programmes under the number IDDN FR 001 120020 00 R P 2000 000 10800, on 15 March 2000        


MUMPS
=======
mumps-solver.org
**License and link to online license text (if existing):**
CeCILL-C Licence
cecill.info/licences/Licence_CeCILL-C_V1-en.html
**Copyright** 
The copyrights of this software are owned by CERFACS, CNRS, ENS Lyon, INP Toulouse, Inria, Mumps Technologies, University of Bordeaux


nibabel
=======
github.com/nipy/nibabel/tree/master
**License and link to online license text (if existing):**
MIT license
github.com/nipy/nibabel/blob/master/COPYING
**Copyright** 
Copyright (c) 2009-2019 Matthew Brett <matthew.brett@gmail.com>
Copyright (c) 2010-2013 Stephan Gerhard <git@unidesign.ch>
Copyright (c) 2006-2014 Michael Hanke <michael.hanke@gmail.com>
Copyright (c) 2011 Christian Haselgrove <christian.haselgrove@umassmed.edu>
Copyright (c) 2010-2011 Jarrod Millman <jarrod.millman@gmail.com>
Copyright (c) 2011-2019 Yaroslav Halchenko <debian@onerussian.com>
Copyright (c) 2015-2019 Chris Markiewicz <effigies@gmail.com>


NumPy
=======
numpy.org
**License and link to online license text (if existing):**
BSD 3-clause-likelicense
numpy.org/doc/stable/license.html
**Copyright** 
Copyright (c) 2005-2024, NumPy Developers


PETSc
=======
petsc.org/release/
**License and link to online license text (if existing):**
2-clause BSD license
petsc.org/release/install/license/
**Copyright** 
Copyright (c) 1991-2024, UChicago Argonne, LLC and the PETSc Development Team
**Remarks** 
please see the PETsc wheel repository on the SimNIBS github for a list of the included dependencies 


pygpc
=======
github.com/pygpc-polynomial-chaos/pygpc
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 3
github.com/pygpc-polynomial-chaos/pygpc/blob/master/LICENSE
**Copyright** 
Konstantin Weise


PyQt5
=======
www.riverbankcomputing.com/software/pyqt/intro
**License and link to online license text (if existing):**
GNU General Public License (GPL), Version 3
or a commercial License
www.riverbankcomputing.com/commercial/license-faq
**Copyright** 
(c) Riverbank Computing Limited
**Remarks** 
used by simnibs_gui


Python
=======
www.python.org
**License and link to online license text (if existing):**
PSF license and
Zero-Clause BSD license
docs.python.org/3/license.html#bsd0
docs.python.org/3/license.html#psf-license
**Copyright** 
Python Software Foundation


pyvista
=======
pyvista.org
**License and link to online license text (if existing):**
MIT license
github.com/pyvista/pyvista/blob/main/LICENSE
**Copyright** 
Copyright (c) 2017-2024 The PyVista Developers
**Remarks** 
used in vtk reader and writer


Qt
=======
www.qt.io
**License and link to online license text (if existing):**
Qt is available under different licensing options designed to accommodate the needs of our various users:
Qt licensed under commercial licenses is appropriate for development of proprietary/commercial software where you do not want to share any source code with third parties or otherwise cannot comply with the terms of the GNU LGPL version 3.
Qt licensed under the GNU Lesser General Public License (LGPL) version 3 is appropriate for the development of Qt applications provided you can comply with the terms and conditions of the GNU LGPL version 3 (or GNU GPL version
Qt components licensed under the Qt Marketplace License Agreement are appropriate for the development of Qt applications commonly with Qt software components licensed under the commercial or GNU LGPL version 3 (or GNU GPL version 3) terms and conditions.
doc.qt.io/qt-6/licensing.html
**Copyright** 
The Qt Company
**Remarks** 
used by simnibs_gui


SCIKIT-IMAGE
===========
scikit-image.org
**License and link to online license text (if existing):**
BSD-3-Clause
scikit-image.org/docs/dev/license.html
**Copyright** 
Copyright (C) 2011, the scikit-image team
**Remarks** 
Marching cube algorithm copied from SCIKIT-IMAGE


SciPy
=======
scipy.org
**License and link to online license text (if existing):**
BSD 3-Clause License
github.com/scipy/scipy/blob/main/LICENSE.txt
**Copyright** 
Copyright (c) 2003-2024, SciPy Developers
Copyright (c) 2001-2002 Enthought, Inc


ZLIB
=======
github.com/madler/zlib
**License and link to online license text (if existing):**
MIT-like license
github.com/madler/zlib?tab=License-1-ov-file#readme
**Copyright** 
Copyright (C) 1995-2017 Jean-loup Gailly and Mark Adler
