...
// file created with getdp.py on 20160304_131018
...
Function {
	...
	sigma[Electrode_rubber] = 0.1;
	sigma[Saline] = 1.0;
}


Function {
	perpendicular_current_density[] = ScalarField[XYZ[]]{0};
}


Jacobian {
	{ Name Volume;  Case {{ Region Omega; Jacobian Vol;  }} }
	{ Name Surface; Case {{  Region Gama;  Jacobian Sur; }} }
}
...
Formulation {{
	...
	Equation {
		Galerkin {
			[sigma[] * 1e3*Dof{Grad v}, {Grad v}];
			In Omega;
			Jacobian Volume;
			Integration GradGrad;
		}
		Galerkin {
			[perpendicular_current_density[], {v}];
			In Gama;
			Jacobian Surface;
			Integration GradGrad;
		}
